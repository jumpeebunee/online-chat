...
import { ActiveUser, IData } from "../types/types";
...
const MessagePage = () => {
  ...
  const openDialog = (userDialog: string) => {
    if (currentUser.uid) {
      return currentUser.uid > userDialog
        ? currentUser.uid + userDialog
        : userDialog + currentUser.uid;
    }
  };
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (doc) => {
        ...
        if (chatsData) {
          setChats(
            Object.entries(chatsData as DocumentData[]).sort(
  ...
  return (
    <section className="main-section">
      <div>
        <MessageSearch setUser={setUser} setErr={setErr} />
